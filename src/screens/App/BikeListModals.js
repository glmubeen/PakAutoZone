...
const Index = ({navigation, route, ...props}) => {
  ...
  const _getBikeBrand = () => {
    setIsLoader(true);
    axios
      .get(BaseURL.BIKE_BRAND_MODAL + data.brand + '/m/' + data._id + '/page/1')
      .then(res => {
            ...
            renderItem={({item}) => {
              return (
                <TouchableOpacity
                  ...
                  className={
                    'bg-white rounded-lg flex self-center p-2 pt-3 pb-3 items-center flex-row mt-3 overflow-hidden'
                  }>
                  <Image
                  ...
                  />
                  <View className={'ml-4'}>
                    <Text className={'text-sm text-black font-medium mb-2'}>
                      {item.title}
                    </Text>
